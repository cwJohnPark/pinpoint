...
To enable the webhook sender service, you need to set **webhook.enable** and **webhook.receiver.url** in [batch/src/main/resources/batch-root.properties/batch-root.properties](https://github.com/pinpoint-apm/pinpoint/blob/master/batch/src/main/resources/batch-root.properties) and set same as in [web/src/main/resources/batch-root.properties](https://github.com/pinpoint-apm/pinpoint/blob/master/web/src/main/resources/batch-root.properties). The webhook.enable means turning on or off the webhook alarm service and webhook.receiver.url means to which a receiver receive the alarm message.
...
Lastly, you should set the type of the alarm rule to webhook in the Pinpoint alarm page. When you set webhook.enable to true, you can see the webhook option on the alarm setting page.
...
Pinpoint-2.1.1 이전에 MySQL 스키마를 생성하였다면 , Pinpoint의 Mysql 스키마를 아래와 같이 변경해야 한다.
...
webhook 전송 기능을 활성화 하려면, 배치 모듈의 [batch/src/main/resources/batch-root.properties/batch-root.properties](https://github.com/pinpoint-apm/pinpoint/blob/master/batch/src/main/resources/batch-root.properties) 파일과 webhook 전송 유무인 **webhook.enable** 과 webhook을 받을 receiver 정보인 **webhook.receiver.url** 의 설정을 추가한다.  또한, 웹 모듈의 [web/src/main/resources/batch-root.properties](https://github.com/pinpoint-apm/pinpoint/blob/master/web/src/main/resources/batch-root.properties) 파일의 속성 중 webhook.enable을 true로 설정한다.
...
마지막으로, Pinpoint의 Alarm 페이지에서 alarm rule 추가시 type을 webhook으로 설정한다. 이 설정을 변경하고 Pinpoint-web을 실행하면 alarm 설정 추가시 type 중에서 webhook이라는 새로운 옵션이 추가된다.
